...
3. **Push blocked on credentials.** No `gh` CLI, no GitHub entry in the macOS keychain, no SSH keys — nothing on this machine can authenticate to GitHub. Everything is committed locally with the remote history merged in (local `main` is ahead of `origin/main` by 3 commits, no conflicts left, tests green). **To publish:** authenticate once (`gh auth login`, or a PAT via `git push` from a terminal) and run `git push -u origin main` inside the tt-tracker folder. The workflow files land with that push, which completes the "finish the workflow via git" item.
- 2026-07-10 evening: **Cloud pipeline verified live.** gh CLI authed via device flow (owner approved in browser), repo pushed + set PRIVATE, first Actions run green end-to-end: rankings 952 players, odds endpoint SERVES bet365 lines from GitHub's runners (region-gating was local only), 1 WTT bet auto-logged (Leo de Nodrest @ -357), czech pipeline scanned 1263 matches / 119 Elo updates / auto-settled both czech bets (both LOST, circuit ROI -2.00u — the honest scoreboard working as designed). Discord notifier deployed; webhook secret still pending (owner not signed into Discord in this browser). Schedule: every 2h (private-repo Actions quota).
